...
function onCheckboxChange(d) {
  ...
  if (isBoxChecked) {
    nextCheckedValues = [
      ...state.filters.checked.slice(0, index),//first we slice before and after, then we use spreader to scotch tape together. spreader is like "keep whatever is in here and do something with it"
  ...
  setState({
    filters: {
      ...state.filters,
      checked: nextCheckedValues,//note that filters contains two parts: checked and menu. we aren't changing/specifying what we want to happen to menu though, just checked. (can access menu without changing what is happening in menu. we manipulate the one part we want, then yada yada the rest -- whatever it is can stay.) it's like an isomer!
    },
  });
}
