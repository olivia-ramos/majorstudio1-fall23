...
//more efficient to nest things like below within json object rather that inidividual lines; would lose track of where menu, checked etc belonged! know it's attached to filters here. 
let state = {
  filters: {
    menu: checkboxValues,
...
async function dataLoad() {
  ...
  source.forEach(
    (d, i) => { // d is the rows from 0 - 150, i keeps count of each one
       d.id = d[CLASS] + "_" + i; // setting a new value bc need a unique ID here; means each entry is now identifiable. iris-setosa-1, iris-setosa-2, etc.
    }
  )
...
function onCheckboxChange(d) {
  ...
  const index = state.filters.checked.indexOf(d.target.name); // first determine index. doesn't matter name of label, just index
  ...
  if (index > -1) {
    ...
    nextCheckedValues.push(d.target.name); // index of checks the existance of an item, if it doesn't exist will show -1 in console.log
  }
