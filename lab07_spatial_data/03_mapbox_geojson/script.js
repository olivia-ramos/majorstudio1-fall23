...
d3.json('africa.geo.json')
  .then((geojson) => {
    ...
    mapboxgl.accessToken = 'pk.eyJ1Ijoib2xpdmlhcmFtb3MiLCJhIjoiY2xueGZxYm1pMGNteDJqcWgxNmZjb3gzcyJ9.IhSPsoLxXBkOLE9ZOIArAA'
    ...
    let featureElement = svg
        ...
        .on('mouseover', function (d) {
          ...
          d3.select('#hover').attr('fill-opacity', 1) // this is for hover; make opacity 1, when not on hover opacity is zero
        })
    ...
    let update = () => {
        featureElement.attr('d', path)// if you grab map and move or scroll left & right or zoom, the path changes. below we're specifying that everytime something happens we want to update the veiw. 
    }
